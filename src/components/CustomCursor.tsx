    ...
    >
      <div
        className={`-translate-x-1/2 -translate-y-1/2 rounded-full flex items-center justify-center transition-all duration-300 ${
          isHovered
            ? 'w-16 h-16 bg-[#00f0ff] text-black shadow-[0_0_25px_rgba(0,240,255,0.6)] scale-110'
            : 'w-4 h-4 bg-white/90 shadow-[0_0_10px_rgba(255,255,255,0.5)]'
        }`}
      >
        {isHovered && cursorText && (
          <span className="text-[10px] font-extrabold tracking-widest font-mono uppercase text-center leading-none px-1">
            {cursorText}
          </span>
        )}
      </div>
